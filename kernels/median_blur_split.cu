...
__device__ unsigned char sorting_network9_split(unsigned char* window){
...
//For further testing
__global__ void medianBlurKernel_split(unsigned char* in, unsigned char* out,int width, int height, int channels, int blur_size) {
    ...
    if(col<width && row<height){
            ...
            //Compute median
            out[(row * width + col) * channels + channel] = sorting_network25_split(window);
    }
}
...
torch::Tensor median_blur_split(torch::Tensor img, int blur_size){
    ...
    for(int i=0; i<stream_count; ++i){
        ...
        medianBlurKernel_split<<<dimGrid, dimBlock, shared_memory_size, streams[i]>>>(
